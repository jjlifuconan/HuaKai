...
public class RxSaveImage {
    ...
    public static void saveImageToGallery(Activity context, String mImageUrl, String mImageTitle) {
        ...
        Subscription s = RxSaveImage.saveImageAndGetPathObservable(context, mImageUrl, mImageTitle)
                ...
                .subscribe(uri -> {
                    File appDir = new File(Environment.getExternalStorageDirectory(), "下载相册");
                    String msg = String.format(CommonUtils.getString(context, R.string.picture_has_save_to),
                            appDir.getAbsolutePath());
                    ToastUtil.showLong(context, msg);
                }, error -> ToastUtil.showLong(context, error.getMessage()));
        // @formatter:on
//        addSubscription(s);
    }
