...
//    }
     ...
     * @param name 键名
     */
    public static void clearAllByFileName(Context context, String name) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(name, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.clear();
        editor.apply();
    ...
    public static void saveObject(Context context, String fileName, String key, Object object){
        ...
        try {
            ...
            String value = new String(Base64.encode(baos.toByteArray(), Base64.DEFAULT));
            sp.edit().putString(key, value).commit();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            IOUtils.closeQuietly(baos,out);
        }
    }

    public static <T> T getObject(Context context, String fileName, String key, Class<T> clz) {
        SharedPreferences sp = context.getSharedPreferences(fileName,context.MODE_PRIVATE);
        if (sp.contains(key)) {
